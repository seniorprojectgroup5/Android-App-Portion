package com.example.buzzinbees;

public class Constant {
    //Constant Java class is used for values that appear in two or more java classes across the app

    public static final int FRAGVAL_HOME = 0;
    public static final int FRAGVAL_SONGS = 1;
    public static final int FRAGVAL_PLAYLISTS = 2;
    public static final int FRAGVAL_ALBUMS = 3;
    public static final int FRAGVAL_ARTISTS = 4;
    public static final int FRAGVAL_VISUALIZER = 5;
    public static final int FRAGVAL_OPTIONS = 6;
    public static final int FRAGVAL_PLAYLISTPAGE = 7;
    public static final int FRAGVAL_ALBUMPAGE = 8;
    public static final int FRAGVAL_ARTISTPAGE = 9;
    public static final int FRAGVAL_NEWPLAYLIST = 10;
    public static final int FRAGVAL_SONGTOPLAYLIST = 11;

    public static final float HEX1SCALE = 0.7087f;
    public static final float HEX2SCALE = 0.5748f;
    public static final float HEX3SCALE = 0.5354f;
    public static final float HEX4SCALE = 0.3386f;

    public static final float HEX1MIN = 0.3544f;
    public static final float HEX2MIN = 0.2874f;
    public static final float HEX3MIN = 0.2667f;
    public static final float HEX4MIN = 0.1693f;

    public static final float HEX1MAX = 0.95f;
    public static final float HEX2MAX = 0.80f;
    public static final float HEX3MAX = 0.75f;
    public static final float HEX4MAX = 0.55f;

}
